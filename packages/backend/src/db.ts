...
// [ 섹션 시작: 임시 타입 정의 ]
// shared 임포트에서 Goal을 빼고, 아래에 직접 정의합니다.
...
async function _getMemFuncs(mes_id: string): Promise<any[]> {
  return select(`
    SELECT B.MEF_ID,
           B.MEF_NAME
      FROM T_MEMBERSHIP_DETAIL A
      JOIN T_MEMBERSHIP_FUNC B ON A.MEF_ID = B.MEF_ID
     WHERE A.MES_ID = :1
  `, [mes_id]);
}

export const getMemFuncs = async (mes_id: string): Promise<MemFunc[]> => {
  const memFuncs = await _getMemFuncs(mes_id);

  return memFuncs.map((func: any) => ({
    MEF_ID: func.MEF_ID,
    MEF_NAME: func.MEF_NAME
  }));
};
...
export const getMemberships = async (): Promise<Membership[]> => {
  const memberships = await _getMemberships();

  return memberships.map((membership: any) => ({
    MES_ID: membership.MES_ID,
    MES_NAME: membership.MES_NAME,
    MES_FEE: membership.MES_FEE
  }));
};
...
async function getRawMember(memberId: string): Promise<any> {
  return select(`
    SELECT
        ...
        B.MES_NAME
    FROM MEMBER A
    -- 💡 B.ID를 B.MES_ID로 바꿔보세요 (T_MEMBERSHIP의 PK 컬럼명 확인!)
...
export const getMember = async (memberId: string): Promise<Member> => {
  ...
  return {
    ...
    mes_id: m.MES_ID || m.mes_id,
    mes_name: m.MES_NAME || m.mes_name
  };
};
...
// [ db.ts - getMemberGoals 수정 ]
export const getMemberGoals = async (memberId: string, date: string): Promise<Goal[]> => {
  const sql = `
    SELECT
      GOL_ID, MEM_ID, GOL_TITLE, GOL_TYPE, ICON_NAME,
      ICON_COLOR, TARGET_VAL, CURRENT_VAL, UNIT,
      IS_REPRESENTATIVE, IS_ACHIEVED,
      GOL_DATE
    FROM T_MEMBER_GOAL
    WHERE MEM_ID = :1
    AND TO_CHAR(GOL_DATE, 'YYYY-MM-DD') = :2
  `;

  const records = await select(sql, [memberId, date]);

  // DB 컬럼(대문자) -> Goal 인터페이스(소문자/카멜케이스) 매핑
  return records.map((rec: any) => ({
    id: rec.GOL_ID,
    iconName: rec.ICON_NAME,
    title: rec.GOL_TITLE,
    current: Number(rec.CURRENT_VAL || 0),
    target: Number(rec.TARGET_VAL || 0),
    unit: rec.UNIT,
    iconColor: rec.ICON_COLOR,
    isRepresentative: rec.IS_REPRESENTATIVE === 'Y',
    isAchievedToday: rec.IS_ACHIEVED === 'Y',
    goalType: rec.GOL_TYPE,
    gol_date: new Date(rec.GOL_DATE)
  })) as Goal[]; // 👈 여기서 타입을 강제로 지정해주면 빨간 줄이 사라질 수 있습니다.
};

// 2. 새로운 목표 추가
export const addMemberGoal = async (goal: any) => {
  const sql = `
    INSERT INTO T_MEMBER_GOAL
    (MEM_ID, GOL_TITLE, GOL_TYPE, ICON_NAME, ICON_COLOR, TARGET_VAL, UNIT, IS_REPRESENTATIVE, GOL_DATE)
  ...
  return await execute(sql, [
    ...
    goal.target_val,
    goal.unit,
    goal.is_representative ? 'Y' : 'N', // boolean을 DB용 문자열로
    goal.gol_date // '2026-03-26' 형식
  ]);
};

// 3. 목표 삭제 (컬럼명 GOL_ID 확인)
export const deleteMemberGoal = async (goalId: number) => {
  const sql = `DELETE FROM T_MEMBER_GOAL WHERE GOL_ID = :1`;
...
export const getAllMemberships = async (): Promise<any[]> => {
  // 1. 모든 멤버십 정보 조회
  const memberships = await select(`SELECT id, name, fee FROM MEMBERSHIP WHERE ID <> 'MS00001' ORDER BY fee`);
